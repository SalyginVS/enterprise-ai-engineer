...
**Tool Use и Function Calling (синонимы).** Механизм, который превращает
языковую модель в агента способного взаимодействовать с внешними системами.
Модель не выполняет код — она принимает решение о том, какую функцию вызвать
и с какими аргументами, и возвращает это решение в структурированном виде
(поле `tool_calls` в ответе). Код разработчика получает этот запрос,
выполняет реальную функцию, возвращает результат модели — и тогда модель
формулирует финальный ответ. Ollama поддерживает tool use через endpoint
`/api/chat` с параметром `tools` в формате JSON Schema.

**Agent Loop (цикл агента).** Основной паттерн построения агентов: цикл
повторяется пока модель запрашивает инструменты, и завершается когда модель
возвращает текстовый ответ без `tool_calls`. Каждая итерация цикла — это
один HTTP-запрос к модели. Для задачи требующей двух инструментов потребуется
минимум три запроса: первый (решение вызвать tool_1), второй (решение вызвать
tool_2 с данными из tool_1), третий (финальный ответ). Защита от зацикливания
через `MAX_ITERATIONS` — обязательный элемент любого agent loop.

**Недетерминизм при tool calling.** В отличие от text generation, где
недетерминизм проявляется в стиле ответа, при tool calling он проявляется
в стратегии вызова инструментов: модель может вызвать их последовательно
или параллельно, с правильными или галлюцинированными аргументами. Один
и тот же вопрос при повторных запусках может дать разный порядок вызовов.
Следствие: аргументы инструментов нельзя считать доверенными — их необходимо
валидировать через Pydantic до исполнения, особенно для деструктивных операций.
...
| EXP-008 | Первый Tool Call | Модель не выполняет — она только запрашивает |
| EXP-009 | Agent Loop, два инструмента | Недетерминизм стратегии вызовов, галлюцинация аргументов |
| EXP-010 | Валидация + System Prompt | Pydantic до execute + fallback на пустой ответ |
| EXP-011 | CustomerSupportAgent | Первый полноценный агент: телеметрия, 3 инструмента |

### Связь с последующими модулями

**Из EXP-004..007 в M1:** Pydantic-валидация закроет проблему невалидных
категорий (EXP-005), структурный escalation заменит ненадёжный confidence
(EXP-007), retry-логика обработает нестабильный JSON (EXP-004). Smart
Pipeline из EXP-007 станет архитектурным скелетом Invoice Classifier Agent.

**Из EXP-008..011 в M1:** Pydantic-валидация аргументов инструментов
(EXP-009, EXP-010) станет стандартом для всех tool calls. Fallback на
пустой ответ (EXP-010) войдёт в базовую архитектуру агента. Телеметрия
из CustomerSupportAgent (EXP-011) — прообраз Prometheus-метрик.
Базовая метрика avg_iterations_per_request = 2.1 фиксируется для
сравнения с M1-агентом.

**Из EXP-009..011 в M2:** Обнаруженный паттерн недетерминизма при
параллельных tool calls (EXP-009) — прямой путь к теме red-teaming
в M2: что происходит если модель галлюцинирует аргументы деструктивного
инструмента? Ответ — именно там.

**Из EXP-011 в M3:** Отсутствие conditional routing в кейсе «жалоба»
(classify → search_faq вместо classify → escalate) — точная формулировка
задачи для M3. Там этот паттерн получит полноценную реализацию через
message-based coordination между агентами.
...
**Статус:** PASSED — все задания выполнены, 12 экспериментов задокументированы

### Что было запланировано и что получилось

Программа планировала M0 как недельный boot-camp с четырьмя заданиями:
environment setup, prompt engineering, tool use, simple agent. Все четыре
выполнены. Но практика добавила сверх плана: три итерации одного агента
(EXP-008 → EXP-009 → EXP-010) дали возможность столкнуться с реальными
проблемами production-уровня — недетерминизмом, галлюцинацией аргументов,
пустыми ответами — и последовательно их решить до перехода к M1.

### Ключевые технические инсайты

**Про LLM как инструмент.** Модель — это вероятностная функция, а не
детерминированный алгоритм. Одинаковый промпт при повторных запусках
может дать разную стратегию поведения. Это не баг — это фундаментальное
свойство, которое определяет всю архитектуру надёжных агентов: валидация,
fallback, защита от зацикливания существуют именно потому, что модель
недетерминирована.

**Про tool calling.** Модель не выполняет инструменты — она только
формулирует запрос на их выполнение. Весь execution остаётся на стороне
кода. Это граница ответственности и граница безопасности одновременно:
именно здесь должны жить access control, валидация и аудит-лог.

**Про prompt engineering.** System prompt — это контракт с моделью,
не инструкция для пользователя. Слишком жёсткий контракт ломает граничные
кейсы (EXP-010, overcorrection). Слишком мягкий — не предотвращает
галлюцинации. Каждое изменение промпта требует прогона всей тест-матрицы.

**Про confidence.** LLM не умеют честно оценивать собственную уверенность.
Confidence=0.95 на очевидном и пограничном кейсах одинаков. Надёжная
валидация строится на структурных признаках (Pydantic enum, формат ответа),
а не на числовом confidence.

### Что берём в M1 — технический долг

| Проблема | Обнаружена | Решение в M1 |
|----------|-----------|--------------|
| Невалидные категории (hallucination) | EXP-005 | Pydantic enum на выходе |
| Нестабильный JSON | EXP-004 | Retry + json parser |
| Ненадёжный confidence | EXP-004, EXP-007 | Структурная валидация |
| Пустой финальный ответ | EXP-009, EXP-010 | Fallback механизм |
| Галлюцинация аргументов tool | EXP-009 | Pydantic до execute_tool |
| Отсутствие conditional routing | EXP-011 | M3 (не M1) |

### Базовые метрики на старте программы

Эти цифры фиксируются как baseline для сравнения с результатами M1:

| Метрика | Значение | Контекст |
|---------|---------|---------|
| Скорость генерации qwen3:14b | ~65 tok/s | RTX 3090, загруженная модель |
| avg_iterations_per_request | 2.1 | CustomerSupportAgent, 7 запросов |
| fallback rate | ~7% | 1 из 14 запросов вернул пустой content |
| validation_errors | 0 | При корректных входных данных |
| Время ответа (простой FAQ) | 5-8с | Один tool call + генерация |

### Личное наблюдение

Самый ценный момент M0 — не когда всё работало, а когда EXP-009 показал
галлюцинацию аргументов при параллельном вызове инструментов. Это живая
иллюстрация того, почему production AI-системы требуют defensive programming:
модель уверенно передала выдуманные значения (10000 RUB вместо 32100 UAH)
и получила технически корректный результат, который был при этом полностью
неверным. Без валидации это прошло бы незамеченным.